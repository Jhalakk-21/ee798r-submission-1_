...
def parse_args():
    ...
    path_temp_trainednet = 'temp/TrainedNet'
    ab_path_temp_trainednet = os.path.abspath(path_temp_trainednet)
    parser.add_argument('--train_dir', type=str, default='/content/drive/MyDrive/Colab Notebooks/CMHF-net/temp/TrainedNet',
...
def train():
    path_row_cave_resp_coeff = 'rowData/CAVEdata/response coefficient'
    ab_path_prcrc = os.path.abspath(path_row_cave_resp_coeff)
    # Prepare data
    data = sio.loadmat('/content/drive/MyDrive/Colab Notebooks/CMHF-net/rowData/CAVEdata/response coefficient.mat')
    R = data['R']
    C = data['C']
    Crd.PrepareDataAndiniValue(R, C, FLAGS.Prepare)
    random.seed(1)

    path_cave_iniA = 'CAVEdata/iniA'
    ab_path_cave_iniA = os.path.abspath(path_cave_iniA)
    iniData1 = sio.loadmat('/content/drive/MyDrive/Colab Notebooks/CMHF-net/CAVEdata/iniA.mat')
    iniA = iniData1['iniA']

    path_cave_iniUp = 'CAVEdata/iniUp'
    ab_path_cave_iniUp = os.path.abspath(path_cave_iniUp)
    iniData2 = sio.loadmat('/content/drive/MyDrive/Colab Notebooks/CMHF-net/CAVEdata/iniUp.mat')
    iniUp3x3 = iniData2['iniUp1']
...
def testAll():
    ...
    path_x = 'CAVEdata/X/'
    ab_path_x = os.path.abspath(path_x)
    path_y = 'CAVEdata/Y/'
    ab_path_y = os.path.abspath(path_y)
    path_z = 'CAVEdata/Z/'
    ab_path_z = os.path.abspath(path_z)
    with tf.Session(config=config) as sess:        
        ckpt = tf.train.latest_checkpoint(save_path)
        saver.restore(sess, ckpt) 
        for root, dirs, files in os.walk(ab_path_x):
            for i in range(32):       
                data = sio.loadmat(ab_path_y + files[i])
                inY  = data['RGB']
                inY  = np.expand_dims(inY, axis = 0)
                data = sio.loadmat(ab_path_z + files[i])
                inZ  = data['Zmsi']
