...
def all_train_data_in():
    ...
    allDataX = []
    allDataY = []
    path_cave_list = 'CAVEdata/List'
    ab_path_cave_list = os.path.abspath(path_cave_list)
    List = sio.loadmat('/content/drive/MyDrive/Colab Notebooks/CMHF-net/CAVEdata/List')
    Ind  = List['Ind'] # a list of rand index with frist 20 to be train data and last 12 to be test data
    path_cave_x = 'CAVEdata/X'
    ab_path_cave_x = os.path.abspath(path_cave_x)
    path_cave_y = 'CAVEdata/Y'
    ab_path_cave_y = os.path.abspath(path_cave_y)
    for root, dirs, files in os.walk('/content/drive/MyDrive/Colab Notebooks/CMHF-net/CAVEdata/X/'):
           for j in range(20):
#                print(Ind[0,j])
                i = Ind[0,j]-1
                data = sio.loadmat('/content/drive/MyDrive/Colab Notebooks/CMHF-net/CAVEdata/X/' + files[i])
                inX  = data['msi']
#                print(type(inX[1,1,1]))
                allDataX.append(inX)
                data = sio.loadmat('/content/drive/MyDrive/Colab Notebooks/CMHF-net/CAVEdata/Y/' + files[i])
                inY  = data['RGB']
                allDataY.append(inY)
...
#    if not folder:
    if prepare != 'No':
        ...
        path_row_cave_complete_msdata = 'rowData/CAVEdata/complete_ms_data/'
        ab_path_row_cave_complete_msdata = os.path.abspath(path_row_cave_complete_msdata)
        for root, dirs, files in os.walk('/content/drive/MyDrive/Colab Notebooks/CMHF-net/rowData/CAVEdata/complete_ms_data'):
            print("in os walk")
            for i in range(32):
                n=n+1
                Z = np.zeros([16,16,31])
...
#                else:
                X = readImofDir('/content/drive/MyDrive/Colab Notebooks/CMHF-net/rowData/CAVEdata/complete_ms_data' + dirs[Ind[i]-1]+'/'+dirs[Ind[i]-1])/255
